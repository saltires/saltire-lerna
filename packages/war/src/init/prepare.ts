...
export default async function prepare(state: Context) {
  ...
  const tempScriptDir = path.join(config.paths.temp, '/scripts', config.packageInfo.name);
  const tempTemplateDir = path.join(config.paths.temp, '/template', config.packageInfo.name);
